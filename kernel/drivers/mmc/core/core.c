...
static void mmc_power_up(struct mmc_host *host)
{
	 ...
	 */
	mmc_delay(100);		// 10 changed by sinclair.lee 20120104 to improve SD card detection rate
...
#ifdef CONFIG_M7LAB_VENUS_SDCARD	// 20120109 by sinclair.lee to improve SD card detection rate
struct mmc_host *mmc_host_juke = (struct mmc_host *)0;

int mmc_rescan_for_juke(unsigned long param)
{
	struct mmc_host *host = mmc_host_juke;

	if (host)
	{
		if(param == 0x9042 || mmc_send_status(host->card, NULL))
		{
			if (host->bus_ops && host->bus_ops->detect && !host->bus_dead
...
#ifdef CONFIG_M7LAB_VENUS_SDCARD	// by sinclair.lee 20111231 to improve sdcard detection
	else if (host->index == 0) {	// sdcard slot??
		if(host->unused && (!host->card || !host->card->state))	// equiped? if physically equiped but not detected yet, retry again
