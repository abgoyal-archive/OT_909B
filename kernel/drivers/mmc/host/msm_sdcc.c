...
msmsdcc_check_status(unsigned long data)
{
	...
	if (!host->plat->status) {
		...
		host->mmc->unused = (unsigned short)status;	// equiped?
#endif
...
#ifdef CONFIG_M7LAB_VENUS_SDCARD	//	added by sinclair.lee 20111230 for debouncing
	if (host->plat->status) {
		int i, cnt, status, old_status = host->plat->status(mmc_dev(host->mmc));
		for(i=0,cnt=0; i<10&&cnt<5; i++)	// debouncing & take care of odd situation(pin toggle)
		{
			udelay(600);
			status = host->plat->status(mmc_dev(host->mmc));
			if(old_status == status) cnt++;
			else cnt = 0;
			old_status = status;
		}
	}
		...
		const char *buf, size_t count)
{
	...
	printk("#### Hugh enter [%s] value =%d, mmcindex=%d,mmcrescan_disable[%d]\n",__FUNCTION__,value,mmc->index,mmc->rescan_disable);
...
static void msmsdcc_late_resume(struct early_suspend *h)
{
	struct msmsdcc_host *host =
		container_of(h, struct msmsdcc_host, early_suspend);
	unsigned long flags;
// FihtdcCode@20111026 WeiChu begin
	unsigned int status;
...
// FihtdcCode@20111026 WeiChu end
