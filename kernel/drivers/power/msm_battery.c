...
static u32 timeout_timer=(HZ*30);   // 30s timeout
...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	wangjin modified
	if( battery_level == BATTERY_LEVEL_FULL && battery_voltage>=4150 && charger_status == 0)
	{
		msm_batt_info.batt_capacity = 100;
		msm_batt_info.batt_status = POWER_SUPPLY_STATUS_FULL;
		DBG_LIMIT("wangjin: battery_level=FULL with supp=%s \n", supp?supp->name:"NULL");
...
{
	{100,4210},
	{99,4170},    // can't up to this level
	{98,4160},    // passed
	{93,4150},    // passed
	{87,4140},	  // passed
	{83,4130},	  // passed
...
static u32 juke_batt_capacity(u32 current_voltage)
{
	...
	else
	{
		...
		if( (old_voltage > current_voltage) && (old_voltage < current_voltage + 300) )
			current_voltage = old_voltage;
	}
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	for(detect_batt=0; detect_batt<3; detect_batt++)
	{
		msm_batt_update_psy_status();
	}
