...
#ifdef CONFIG_M7LAB_VENUS_BRCM
#define WL_BT_REGON_PIN		PM_MPP_22
#define BT_RESET_PIN		PM_MPP_10
#define BT_MSM_WAKES_BT		PM_MPP_14	// keep below gpio 42 to manupulate wake simply by driver. It's N.C pin
static bool bt_power = false;
static bool wifi_power = false;
#endif
...
static int bluetooth_power(int on)
{
	...
	vreg_bt = vreg_get(NULL, "gp4");	//	Teddy 2011-04-19	GP6=>GP4
...
#ifdef CONFIG_M7LAB_VENUS_BRCM	//	Teddy 2011-07-18
	bt_power = on;
	...
	if (on) {
...
#ifdef CONFIG_M7LAB_VENUS_BRCM	//	Teddy 2011-07-18
		printk(KERN_DEBUG "%s %d mid\n", __func__, on);

		mpp_config_digital_out(BT_RESET_PIN, MPP_CFG(MPP_DLOGIC_LVL_MSMP, MPP_DLOGIC_OUT_CTRL_LOW));
		mdelay(100);
		mpp_config_digital_out(WL_BT_REGON_PIN, MPP_CFG(MPP_DLOGIC_LVL_MSMP, MPP_DLOGIC_OUT_CTRL_HIGH));
		mpp_config_digital_out(BT_RESET_PIN, MPP_CFG(MPP_DLOGIC_LVL_MSMP, MPP_DLOGIC_OUT_CTRL_HIGH));
		gpio_set_value( 42, 1);
#endif	//	CONFIG_M7LAB_VENUS_BRCM
		...
		mpp_config_digital_out(BT_RESET_PIN, MPP_CFG(MPP_DLOGIC_LVL_MSMP, MPP_DLOGIC_OUT_CTRL_LOW));
		if( !wifi_power )
			mpp_config_digital_out(WL_BT_REGON_PIN, MPP_CFG(MPP_DLOGIC_LVL_MSMP, MPP_DLOGIC_OUT_CTRL_LOW));
...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	wangjin, 2012-06-14
	.voltage_min_design		= 0,	//	wangjin, 2012-06-14
...
static struct msm_gpio sdc4_cfg_data[] = {
	...
	//{GPIO_CFG(21, 4, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), "sdc4_dat_1"}, // commented for bug 242048
...
static void sdcc_gpio_init(void)
{
	...
	rc = gpio_tlmm_config(GPIO_CFG(49, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc)
		printk(KERN_ERR "%s: Failed to configure GPIO %d\n",
				__func__, rc);

	if (gpio_request(21, "sdc2_wakeup_irq"))
		pr_err("failed to request gpio sdc2_wakeup_irq\n");
	rc = gpio_tlmm_config(GPIO_CFG(21, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
				GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc)
		printk(KERN_ERR "%s: Failed to configure GPIO %d\n",
				__func__, rc);
...
#ifdef CONFIG_TOUCHSCREEN_FOCALTECH_I2C_5202
#define FT5202_I2C_BUSID	2		// MSM-I2C(0), MSM-AUX-I2C(1), TSP(2)
#define FT5202_I2C_ADDRESS 0x38	// 0x70 for write, 0x71 for read
#define FT5202_I2C_IRQ_PIN	19
#define FT5202_I2C_SCL_PIN	93
#define FT5202_I2C_SDA_PIN	107
...
//#ifndef CONFIG_TOUCHSCREEN_FOCALTECH_I2C_5202_POWER_OFF	// 20110413
		gpio_tlmm_config(GPIO_CFG(FT5202_I2C_SCL_PIN, 0,
			GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_tlmm_config(GPIO_CFG(FT5202_I2C_SDA_PIN, 0,
			GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_tlmm_config(GPIO_CFG(FT5202_I2C_IRQ_PIN, 0,
...
static struct ft_i2c_platform_data ft_i2c_data[] = {
	{
		...
		.abs_y_min = 0,
		.abs_y_max = 320,
		.intr_gpio = FT5202_I2C_IRQ_PIN,
		.power = ft_tsp_power,
		.scl_pin = FT5202_I2C_SCL_PIN,
		.sda_pin = FT5202_I2C_SDA_PIN,
	}
};

static struct i2c_board_info ft_i2c_bdinfo[] = {
	{
		I2C_BOARD_INFO(FT_I2C_NAME, FT5202_I2C_ADDRESS),
		.irq = MSM_GPIO_TO_INT(FT5202_I2C_IRQ_PIN),
...
#ifdef CONFIG_M7LAB_VENUS_SENSOR
//------------------------------- Magentic sensor --------------------------------------
static struct i2c_board_info mmc328x_sensor_i2c_bdinfo[] = {
	{
		I2C_BOARD_INFO(MMC328X_I2C_NAME/*mmc328x*/, MMC328X_I2C_ADDR/*0x30*/),
	},
};

static struct gpio_i2c_pin mmc328x_sensor_i2c_pin[] = {
	{
		.sda_pin	= 76,
		.scl_pin	= 91,
	},
...
static struct i2c_gpio_platform_data mmc328x_sensor_i2c_pdata = {
	.sda_pin = 76,
	.scl_pin =91,
	.sda_is_open_drain = 0,
	.scl_is_open_drain = 0,
...
static struct platform_device mmc328x_sensor_i2c_device = {
	.id = 5,
	.name = "i2c-gpio",
	.dev.platform_data = &mmc328x_sensor_i2c_pdata,
};
//------------------------------- Accelometer sensor --------------------------------------
...
static struct i2c_board_info taos_sensor_i2c_bdinfo[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(20),	//	Teddy 94=>20
		//.platform_data = &ft_i2c_data,
	},
};
...
//end add by peng.xiong	
	...
	i2c_register_board_info(FT5202_I2C_BUSID, &ft_i2c_bdinfo[0], ARRAY_SIZE(ft_i2c_bdinfo));
...
#ifdef CONFIG_M7LAB_VENUS_SENSOR
	//------------------------------- Magentic sensor --------------------------------------
	msm7x27_gpio_i2c_pin(&mmc328x_sensor_i2c_pdata, &mmc328x_sensor_i2c_pin[0], mmc328x_sensor_i2c_bdinfo);
	i2c_register_board_info(5, mmc328x_sensor_i2c_bdinfo, ARRAY_SIZE(mmc328x_sensor_i2c_bdinfo));
	platform_device_register(&mmc328x_sensor_i2c_device);
